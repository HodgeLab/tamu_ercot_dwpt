...
struct MustRunConstraint <: PSI.ConstraintType end
...
function apply_must_run_constraints!(model)
    ...
    commitment_variables = PSI.get_variable(optimization_container, PSI.OnVariable(), ThermalMultiStart);
    ...
    constraint = PSI.add_constraints_container!(
        ...
        MustRunConstraint(),
        ThermalMultiStart,
        PSI.get_name.(must_run_gens),
        time_steps
    )
    jump_model = PSI.get_jump_model(optimization_container)
    for t in time_steps, g in PSI.get_name.(must_run_gens)
        constraint[g, t] =
            JuMP.@constraint(jump_model, commitment_variables[g, t] == 1.0)
    end
end
...
function apply_cc_constraints!(model)
    ...
    #constraint = PSI.get_constraint(optimization_container, PSI.ConstraintKey{CCConstraint, ThermalMultiStart}(""))
    jump_model = PSI.get_jump_model(optimization_container)
